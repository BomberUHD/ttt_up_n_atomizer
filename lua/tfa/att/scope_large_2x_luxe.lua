...
ATTACHMENT.Name = "Luxurious Sniper Scope"
--ATTACHMENT.ID = "base" -- normally this is just your filename
ATTACHMENT.Description = { TFA.AttachmentColors["+"], "5% less spread", TFA.AttachmentColors["+"], "Elegant golden etched patterns", TFA.AttachmentColors["="], "2x zoom", TFA.AttachmentColors["-"], "25% higher zoom time",  TFA.AttachmentColors["-"], "Obstructed field of view" }
ATTACHMENT.Icon = "entities/scope_large_luxe.png" --Revers to label, please give it an icon though!  This should be the path to a png, like "entities/tfa_ammo_match.png"
ATTACHMENT.ShortName = "SCOPE"

local fov = 90 / 2 / 2 -- Default FOV / Scope Zoom / screenscale
...
ATTACHMENT.WeaponTable = {
	...
	["Secondary"] = {
		...
		["ScopeZoom"] = function( wep, val ) return 20 end
	},
	...
	["IronSightsPos"] = function( wep, val ) return wep.IronSightsPos_ScopeLarge2x or val end,
	["IronSightsAng"] = function( wep, val ) return wep.IronSightsAng_ScopeLarge2x or val end,
}
...
ATTACHMENT.FOV = fov
ATTACHMENT.Reticule = "models/krazy/gtav/weapons/sniper_default_reticule"
...
function ATTACHMENT:Attach(wep)
	...
	wep.RTCode = function( myself , rt, scrw, scrh)
		...
		local att, ts
		if wep:VMIV() then
			att = wep.OwnerViewModel:GetAttachment( wep.RTAttachment_ScopeLarge2x or 0 )
		end
		...
		if wep.ScopeAngleTransforms_ScopeLarge then
			ang:RotateAroundAxis(ang:Right(), wep.ScopeAngleTransforms_ScopeLarge2x.p )
			ang:RotateAroundAxis(ang:Up(), wep.ScopeAngleTransforms_ScopeLarge2x.y )
			ang:RotateAroundAxis(ang:Forward(), wep.ScopeAngleTransforms_ScopeLarge2x.r )
		end
		cd.angles = ang
		...
		if ts then
			...
			if wep.ScopeOverlayTransforms_ScopeLarge then
				scrpos.x = scrpos.x + wep.ScopeOverlayTransforms_ScopeLarge2x[1]
				scrpos.y = scrpos.y + wep.ScopeOverlayTransforms_ScopeLarge2x[2]
			end
			...
			if wep.ScopeOverlayTransformMultiplier_ScopeLarge2x then
				scrpos.x = scrpos.x * wep.ScopeOverlayTransformMultiplier_ScopeLarge2x
				scrpos.y = scrpos.y * wep.ScopeOverlayTransformMultiplier_ScopeLarge2x
			end
			...
			local rtow, rtoh = 0, 0
			if wep.RTScopeOffset_ScopeLarge then
				rtow = self.RTScopeOffset_ScopeLarge2x[1] * rttw
				rtoh = self.RTScopeOffset_ScopeLarge2x[2] * rtth
			end
			local rtw, rth = rttw * 1, rtth * 1
			if self.RTScopeScale_ScopeLarge2x then
				rtw = rtw * self.RTScopeScale_ScopeLarge2x[1]
				rth = rth * self.RTScopeScale_ScopeLarge2x[2]
			end
			local distfac = math.pow( 1 - math.Clamp( ( att.Pos:Distance( wep.Owner:GetShootPos() ) - ( wep.ScopeDistanceMin_ScopeLarge2x or 6 ) ) / ( wep.ScopeDistanceRange_ScopeLarge2x or 12 ), 0, 1 ), 1 )
